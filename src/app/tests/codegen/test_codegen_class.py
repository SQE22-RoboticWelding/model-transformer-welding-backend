...
def test_code_generation():
    project_obj = Project(id=1, name="My welding project")
    welding_point_obj = WeldingPoint(project_id=1,
                                     ...
                                     pitch=3,
                                     yaw=0)
    project_obj.welding_points = [welding_point_obj]
    generation_template_obj = GenerationTemplate(id=1, name="My Template", content=get_example_template())

    generated = CodeGenerator.generate(template=generation_template_obj, project=project_obj)
